    ...
    [self.items enumerateObjectsUsingBlock:^(UIView *item, NSUInteger idx, BOOL *stop)
     {
         ...
         {
             [item sizeToFit];
             item.center = CGPointMake(pos + KITEMWIDTH / 2, itemsVAlignCenter);
//        NSLog(@"itemwidth=%f",CGRectGetWidth(item.bounds));
         }
         else
         {
             item.frame = CGRectMake(pos, 0, KITEMWIDTH, itemsVAlignCenter * 2);
//        NSLog(@"itemwidth=%f",CGRectGetWidth(item.bounds));
         }
         pos += KITEMWIDTH + interItemSpace;
     }];
    ...
    else
    {
        ...
        CGRect stainFrame = CGRectInset(selectedItem.frame, -1, -1);
