...
@implementation MRelationAttention

- (void) post:(NSString *)userId completed:(ActionCompletedBlock)completedBlock
{
    [self postRequest:[NSString stringWithFormat:API_RELATION_ATTENTION_ADD_URL] before:^(ASIHTTPRequest *request) {
...
{
    [self deleteRequest:[NSString stringWithFormat:API_RELATION_ATTENTION_DELETE_URL,userId] before:nil completed:^(NSObject *result, BOOL hasError) {
...
{
    [self getRequest:[NSString stringWithFormat:API_RELATION_ATTENTION_LIST_URL,[NSString stringWithFormat:@"%d",page]] before:nil completed:^(NSObject *result, BOOL hasError) {
