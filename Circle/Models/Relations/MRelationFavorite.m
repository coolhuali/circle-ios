...
@implementation MRelationFavorite

- (void) post:(NSString *)userId completed:(ActionCompletedBlock)completedBlock
{
    [self postRequest:[NSString stringWithFormat:API_RELATION_FAVORITE_ADD_URL] before:^(ASIHTTPRequest *request) {
...
- (void) remove:(NSString *)userId completed:(ActionCompletedBlock)completedBlock
{
    ...
    [self deleteRequest:[NSString stringWithFormat:API_RELATION_FAVORITE_DELETE_URL,userId] before:nil completed:^(NSObject *result, BOOL hasError) {
...
{ 
    [self getRequest:[NSString stringWithFormat:API_RELATION_FAVORITE_LIST_URL,[NSString stringWithFormat:@"%d",page]] before:nil completed:^(NSObject *result, BOOL hasError) {
