...
@implementation MRelationBlacklist

- (void) post:(NSString *)userId completed:(ActionCompletedBlock)completedBlock
{
    [self postRequest:[NSString stringWithFormat:API_RELATION_BLACK_ADD_URL] before:^(ASIHTTPRequest *request) {
...
{
    [self deleteRequest:[NSString stringWithFormat:API_RELATION_BLACK_DELETE_URL,userId] before:nil completed:^(NSObject *result, BOOL hasError) {
...
{
    [self getRequest:[NSString stringWithFormat:API_RELATION_BLACK_LIST_URL,[NSString stringWithFormat:@"%d",page]] before:nil completed:^(NSObject *result, BOOL hasError) {
