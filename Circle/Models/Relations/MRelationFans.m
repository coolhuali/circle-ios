...
@implementation MRelationFans

- (void) post:(NSString *)userId completed:(ActionCompletedBlock)completedBlock
{
    [self postRequest:[NSString stringWithFormat:API_RELATION_FANS_ADD_URL] before:^(ASIHTTPRequest *request) {
...
{
    [self deleteRequest:[NSString stringWithFormat:API_RELATION_FANS_DELETE_URL,userId] before:nil completed:^(NSObject *result, BOOL hasError) {
...
{ 
    [self getRequest:[NSString stringWithFormat:API_RELATION_FANS_LIST_URL,[NSString stringWithFormat:@"%d",page]] before:nil completed:^(NSObject *result, BOOL hasError) {
