...
{
    [super viewDidLoad];
    mRelation = [MRelationAttention alloc];
    self.title = NSLocalizedString(@"attention.title", nil);
    [self setupHeader];
    [self setupFooter];
    [self undoEditItems];
...
{
    NSDictionary *item = [items objectAtIndex:indexPath.row];
    UserProfileViewController *control = [[UserProfileViewController alloc] initWithProfileId:[item objectForKey:@"userid"] sourceType:@"sub"];
