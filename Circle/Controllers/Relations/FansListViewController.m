...
{
    [super viewDidLoad];
    mRelation = [MRelationFans alloc];
    self.title = NSLocalizedString(@"fans.title", nil);
    [self setupHeader];
    [self setupFooter];
    [self undoEditItems];
...
{
    NSDictionary *item = [items objectAtIndex:indexPath.row];
    UserProfileViewController *control = [[UserProfileViewController alloc] initWithProfileId:[item objectForKey:@"userid"] sourceType:@"fans"]; 
