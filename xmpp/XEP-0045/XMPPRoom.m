...
enum XMPPRoomConfig
{
	kAutoFetchRoom = 1 << 0,                   // If set, we automatically fetch roster after authentication
	kAutoAcceptInviteRequests = 1 << 1, // See big description in header file... :D
};
enum XMPPRoomState
{
...
- (id)initWithRoomStorage:(id <XMPPRoomStorage>)storage dispatchQueue:(dispatch_queue_t)queue
{
	return [self initWithRoomStorage:storage jid:nil dispatchQueue:queue];
}
...
- (BOOL)autoFetchRoom
{
	...
	dispatch_block_t block = ^{
		result = (config & kAutoFetchRoom) ? YES : NO;
	};
...
- (void)setAutoFetchRoom:(BOOL)flag
{
	dispatch_block_t block = ^{
		
		if (flag)
			config |= kAutoFetchRoom;
		else
			config &= ~kAutoFetchRoom;
	};
...
- (void)fetchRoom
{
	dispatch_block_t block = ^{ @autoreleasepool {
        ...
        [iq addAttributeWithName:@"id" stringValue:fetchID];
        [iq addAttributeWithName:@"to" stringValue:[NSString stringWithFormat:@"conference.%@",xmppStream.hostName ]];
		...
		[responseTracker addID:fetchID
                        target:self
                      selector:@selector(handleFetchRoomListResponse:withInfo:)
                       timeout:60.0];
	}};
...
- (void)handleFetchRoomListResponse:(XMPPIQ *)iq withInfo:(id <XMPPTrackingInfo>)info
{
	if ([[iq type] isEqualToString:@"result"])
	{
        NSXMLElement *queryResult = [iq childElement];
        NSArray *items = [queryResult elementsForName:@"item"];
        for (NSXMLElement *item in items){
            [xmppRoomStorage handleRoomInfo:item room:self];
        }
        [multicastDelegate xmppRoom:self didFetchRoomList:items];
    }else{
		[multicastDelegate xmppRoom:self didNotFetchRoomList:iq];
    }
}
