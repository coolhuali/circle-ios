...
- (NSEntityDescription *)infoEntity:(NSManagedObjectContext *)moc;
...
- (void)commonInit
{
	...
	infoEntityName = NSStringFromClass([XMPPRoomInfoCoreDataStorageObject class]);
...
- (void)setInfoEntityName:(NSString *)newInfoEntityName
{
	dispatch_block_t block = ^{
		infoEntityName = newInfoEntityName;
	};
 ...
 * Optional override hook for general extensions.
 *
 * @see insertInfoWithPresence:room:stream:
 **/
- (void)didInsertInfo:(XMPPRoomInfoCoreDataStorageObject *)occupant
{
	// Override me if you're extending the XMPPRoomOccupantCoreDataStorageObject class to add additional properties.
	// You can update your additional properties here.
	//
 ...
 * Optional override hook for general extensions.
 *
 * @see updateInfo:withPresence:room:stream:
 **/
- (void)didUpdateInfo:(XMPPRoomInfoCoreDataStorageObject *)occupant
{
	// Override me if you're extending the XMPPRoomOccupantCoreDataStorageObject class to add additional properties.
	// You can update your additional properties here.
	//
 ...
 * @see didInsertInfo:
 **/
- (void)insertInfo:(NSXMLElement *)item
                              room:(XMPPRoom *)room
                            stream:(XMPPStream *)xmppStream
{
    ...
    NSString *roomPwd = [item attributeStringValueForName:@"password"];
	// Add to database
	
    NSString *roomSubject = [item attributeStringValueForName:@"subject"];
 	NSManagedObjectContext *moc = [self managedObjectContext];
	NSString *streamBareJidStr = [[self myJIDForXMPPStream:xmppStream] bare];
	
	NSEntityDescription *infoEntity = [self infoEntity:moc];
	
	XMPPRoomInfoCoreDataStorageObject *info = (XMPPRoomInfoCoreDataStorageObject *)
    [[NSManagedObject alloc] initWithEntity:infoEntity insertIntoManagedObjectContext:nil];
	 
	info.roomJID = roomJID;
    info.roomJIDStr = [roomJID full];
    info.roomName = roomName;
    info.roomPwd = roomPwd;
    info.roomSubject = roomSubject; 
    info.myNickname = room.myNickname;
	info.createdAt = [NSDate date];
	info.streamBareJidStr = streamBareJidStr;
	
	[moc insertObject:info];       // Hook if subclassing XMPPRoomOccupantCoreDataStorageObject (awakeFromInsert)
	[self didInsertInfo:info]; // Hook if subclassing XMPPRoomCoreDataStorage
}
 ...
 * @see didUpdateInfo:
 **/
- (void)updateInfo:(XMPPRoomInfoCoreDataStorageObject *)info item:(NSXMLElement *)item room:(XMPPRoom *)room stream:(XMPPStream *)stream
{
	// Extract needed information  
    NSString *roomName = [item attributeStringValueForName:@"name"];
    NSString *roomPwd = [item attributeStringValueForName:@"password"];
    ...
    info.roomName = roomName;
    info.roomSubject = roomSubject;
    info.roomSubject = room.roomSubject;
    info.myNickname = room.myNickname;
	[self didUpdateInfo:info]; // Hook if subclassing XMPPRoomCoreDataStorage
}

/**
 * Optional override hook.
 **/
- (void)removeInfo:(XMPPRoomInfoCoreDataStorageObject *)info
          withPresence:(XMPPPresence *)presence
                  room:(XMPPRoom *)room
                stream:(XMPPStream *)stream
{
...
- (NSEntityDescription *)infoEntity:(NSManagedObjectContext *)moc
{
	...
	return [NSEntityDescription entityForName:[self infoEntityName] inManagedObjectContext:moc];
}
...
- (XMPPRoomInfoCoreDataStorageObject *)infoForJID:(XMPPJID *)jid
                                                   stream:(XMPPStream *)xmppStream
                                                inContext:(NSManagedObjectContext *)moc
{
	if (jid == nil) return nil;
	...
	NSEntityDescription *entity = [self infoEntity:moc];
	...
	{
		predicate = [NSPredicate predicateWithFormat:@"roomJIDStr == %@", jid];
	}
	...
	return (XMPPRoomInfoCoreDataStorageObject *)[results lastObject];
}
...
- (void)handleRoomInfo:(NSXMLElement *)item room:(XMPPRoom *)room
{
	XMPPLogTrace();
	...
	[self scheduleBlock:^{
		...
		// Is roomInfo already in database?
		
		XMPPRoomInfoCoreDataStorageObject *roomInfo =
        [self infoForJID:roomJID stream:room.xmppStream inContext:moc];
		 
		// Insert or update occupant in database
        if (roomInfo)
        {
            [self updateInfo:roomInfo item:item room:room stream:room.xmppStream];
        }
        else
        {
            [self insertInfo:item room:room stream:room.xmppStream];
        }
	}];
}
...
- (void)handleIncomingMessage:(XMPPMessage *)message room:(XMPPRoom *)room
{
	...
	if (msgId==nil||[roomJID isEqualToJID:messageJID])
	{
