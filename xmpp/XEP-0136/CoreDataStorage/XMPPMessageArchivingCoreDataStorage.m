...
- (XMPPMessageArchiving_Message_CoreDataObject *)messageWithMessageID:(NSString *)messageID
                                                            streamJid:(XMPPJID *)streamJid
                                                 managedObjectContext:(NSManagedObjectContext *)moc
{
    XMPPMessageArchiving_Message_CoreDataObject *result = nil;
    ...
    NSString *predicateFmt = @"messageID == %@ AND streamBareJidStr == %@";
    NSPredicate *predicate = [NSPredicate predicateWithFormat:predicateFmt,messageID,[streamJid bare]];
...
- (void)archiveMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing xmppStream:(XMPPStream *)xmppStream
{
    ...
    messageBody = [[NSString alloc] initWithData:data encoding:NSNonLossyASCIIStringEncoding];
	BOOL isComposing = NO;
	...
	[self scheduleBlock:^{
		...
		else
		{
            ...
            archivedMessage.nickname = isOutgoing?@"":[messageJid resource];
            ...
            archivedMessage.isServerDelivered = NO;
            archivedMessage.isMessageDelivered = NO;
...
- (void)archiveReceipt:(XMPPMessage *)receipt xmppStream:(XMPPStream *)stream
{
    [self scheduleBlock:^{
        NSString *messageID = [receipt extractReceiptResponseID];
        XMPPJID *myJID = [stream myJID];
        NSManagedObjectContext *moc = [self managedObjectContext];
        XMPPMessageArchiving_Message_CoreDataObject *archivedMessage = [self messageWithMessageID:messageID
                                                                                        streamJid:myJID
                                                                             managedObjectContext:moc];
        if (archivedMessage) {
            NSString *from = [[receipt from]bare];
            BOOL isServerReceipt = ![from isEqualToString:[archivedMessage bareJidStr]];
            if (isServerReceipt) {
                archivedMessage.isServerDelivered = YES;
            }else{
                archivedMessage.isMessageDelivered = YES;
            }
            NSError *error = nil;
            if(![moc save:&error]){
